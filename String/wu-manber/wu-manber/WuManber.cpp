...
unsigned char WuManber::rchExtendedAscii[] = { 
	...
	bool WuManber::Initialize( const vector<const char *> &patterns, 
			...
			for ( size_t j = 0; j < k; ++j ) {  // loop through patterns
				for ( size_t q = m; q >= B; --q ) {
					...
					if ( 0 == shiftlen ) {
						...
						m_vPatternMap.insert(make_pair(hash,m_PatternMapElement));
					}
				}
			}
			m_bInitialized = true;
	...
	bool WuManber::Search( size_t TextLength, const char *Text, const vector<const char *> &patterns ) {
		...
		while ( ix < TextLength ) {
			...
			if ( shift > 0 ) {
				...
				multimap<unsigned int,structPatternMap>::iterator iter = m_vPatternMap.lower_bound(hash1);
