...
#define MaxLength 8000    //用于优先队列（Open表）的数组
#define Height    600    //地图高度
#define Width     600    //地图宽度
...
cv::Mat map_mat = cv::imread("./map.bmp", 0);
// 优先队列基本操作
...
void initGraph(int map_int[Height][Width], int sx, int sy, int dx, int dy)
{    //地图发生变化时重新构造地
//   cv::Mat element = cv::getStructuringElement(cv::MORPH_RECT, cv::Size(10, 10));
//   cv::dilate(map_mat, map_mat, element);

    for (int k = 0; k < map_mat.rows; ++k) {
        for (int i = 0; i < map_mat.cols; ++i) {
            if(map_mat.at<uint8_t>(k, i) > 90)
                map_int[i][k] = 1;
            else
                map_int[i][k] = 0;
        }
        ...
        for (j = 0; j < Width; j++)
        {
            ...
            if (i > 0)
            {
                ...
                if (j < Width - 1)
                {
                    if (graph[i - 1][j + 1].reachable
                        && graph[i - 1][j].reachable
                        && map_int[i][j + 1] == Reachable) // up-right节点可以到达
                    {
    ...
    {    //times++;
        ...
        for (i = 0; i < 8; i++)
        {
            ...
            if (!close[surX][surY].vis)
            {
                close[surX][surY].vis = 1;
                close[surX][surY].from = p;
                surG = p->G + sqrt((curX - surX) * (curX - surX) + (curY - surY) * (curY - surY)*1.0);
...
int printShortest()
{
    ...
    else
    {
        while (p->from)
        {
            ...
            printf("(%d,%d)->\n", p->cur->x, p->cur->y);
            cv::circle(map_mat, cv::Point(p->cur->x, p->cur->y), 0, 255, -1);
            cv::imshow("1", map_mat);
            cv::waitKey(1);
            p = p->from;
            step++;
...
{    // Clear Map Marks of Steps
    ...
    graph[srcX][srcY].value = map_int[srcX][srcY];
    graph[dstX][dstY].value = map_int[dstX][dstY];
}
...
void on_mouse(int event, int x, int y, int flags, void *ustc)
{
    ...
    else if (event == CV_EVENT_LBUTTONUP)
    {
        ...
        if (within(srcX, srcY) && within(dstX, dstY))
        {
            ...
            {
                printf("from(%d,%d)to(%d,%d) min step is :%d\n",
...
//                printMap();
                ...
                bfs();
                //printDepth();
                puts((shortestep == close[dstX][dstY].G) ? "right" : "false");
                clearMap();
            }
            else
            {
                printf("from(%d,%d)can't arrived(%d,%d)\n",
                       srcX, srcY, dstX, dstY);
            }
        }
        else
...
{
    initGraph(map_int, 0, 0, 0, 0);
//    printMap();
    printf("请输入起点和终点位置\n " );
//    while (scanf("%d %d %d %d", &srcX, &srcY, &dstX, &dstY) != EOF)

    cv::namedWindow("1");
    cv::setMouseCallback("1", on_mouse, 0);
    cv::imshow("1", map_mat);
    cv::waitKey(0);
